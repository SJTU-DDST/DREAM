...
then
    ...
    --server_ip 192.168.98.70,192.168.98.71,192.168.98.73 \
    --gid_idx 1 \
    ...
    for num_cli in `seq $2 $2`;do
        for num_coro in `seq 1 $3`;do
            for load_num in 0;do
                ...
                # ./ser_cli_var_kv \
                ./ser_cli_var_kv \
                --server_ip 192.168.98.70,192.168.98.71,192.168.98.73 --num_machine $4 --num_cli $num_cli --num_coro $num_coro \
                --gid_idx 1 \
                --max_coro 256 --cq_size 64 \
                --machine_id $1  \
                --load_num $load_num \
