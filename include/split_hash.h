...
struct Directory
{
    ...
    void print(){
        ...
        for(uint64_t i = 0 ; i < (1<<global_depth) ; i++){
            log_err("Entry %lu : local_depth:%lu cur_seg_ptr:%lx main_seg_ptr:%lx main_seg_len:%lx",i,segs[i].local_depth,segs[i].cur_seg_ptr,segs[i].main_seg_ptr,segs[i].main_seg_len);
        }
    }
} __attribute__((aligned(8)));
